...
const OrdersTableTools = () => {
    ...
    const salesTopic = `Etat des approvisionement sur la period du
     ${new Date(startDate).toLocaleDateString('en-GB')} au ${new Date(
        endDate
    ).toLocaleDateString('en-GB')}`
    const purchaseExportData = sales?.map((i) => {
        return {
            "Soure d'Aprovisionement": i.income_source,
            Montant: i.amount,
    ...
    const handleSalesExport = (data, topic) => {
        ...
        link.href = url
        link.setAttribute('download', 'Etat des Approvisionement.csv')
